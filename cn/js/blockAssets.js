 ...
 * @param {Object} hash
 */
function getParseContract1(coinaddress,type,pageIndex,startIndex,hash160) {
	if (coinaddress == "") {
		$("#con-box-yu").html("未查到该信息");
		return;
	}
		...
		function(result) {
			if (result.code == "2000") {
				...
				if(type==1){
					...
					getTransferLogList(result.data.address,result.data.hash160,type,pageIndex,coinHash,result.data.code);
